...
export async function POST(req: NextRequest) {
    try {
        ...
        const uploadTarget = formData.get('uploadTarget') as string

        if (!file || !teacherId || !uploadTarget) {
        ...
        const filePath = `teachers/${teacherId}/${uploadTarget.replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}.${fileExt}`
