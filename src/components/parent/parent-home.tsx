...
export function ParentHome() {
                ...
                if (recentGrades) {
                    recentGrades.forEach(g => {
                        activities.push({
                            ...
                            type: 'grade',
                            title: 'Nouvelle note',
                            subject: (g.subjects as { name?: string })?.name || 'Matière',
    ...
    const quickActions = [
        { icon: FileText, label: 'Notes', color: 'text-emerald-400', bg: 'bg-emerald-400/10', href: '/parent/grades' },
        { icon: CreditCard, label: 'Paiements', color: 'text-blue-400', bg: 'bg-blue-400/10', href: '/parent/finances' },
        { icon: Calendar, label: 'Absences', color: 'text-red-400', bg: 'bg-red-400/10', href: '/parent/attendance' },
    ]
        ...
        <div className="max-w-md mx-auto lg:max-w-3xl pb-24 space-y-8 p-4">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Parent</p>
                        <h1 className="text-lg font-bold">Bonjour, {parentName}</h1>
                    </div>
                </div>
            ...
            {childrenList.length === 0 && (
                <div className="bg-card border border-border rounded-3xl p-6 text-center">
                    ...
                    <p className="text-sm text-muted-foreground mt-2">Contactez l'administration pour associer vos enfants.</p>
                </div>
            )}
                ...
                <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-gray-900 to-black border border-white/10 p-6 shadow-2xl">
                    ...
                    <div className="relative z-10 space-y-6">
                        <div className="flex justify-between items-start">
                            <div>
                                <p className="text-muted-foreground text-sm font-medium mb-1">Moyenne Générale ({selectedChild.name})</p>
                        ...
                        <div className="flex gap-4">
                            <div className="flex items-center gap-3 bg-white/5 px-4 py-2.5 rounded-xl border border-white/5 flex-1">
                                ...
                                <div>
                                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Présence</p>
                            ...
                            <div className="flex items-center gap-3 bg-white/5 px-4 py-2.5 rounded-xl border border-white/5 flex-1">
                                ...
                                <div>
                                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Conduite</p>
            ...
            {selectedChild && (
                <div className="space-y-3">
                    <div className="flex items-center justify-between px-1">
                        <h2 className="font-bold text-lg">Prochain Cours ({selectedChild.class})</h2>
                        <Link href="/parent/schedule" className="text-xs text-primary font-medium hover:underline">Voir tout</Link>
                    </div>
                    {loadingExtra ? (
                        ...
                        <div className="bg-card border border-border/50 p-4 rounded-3xl text-center">
                            <p className="text-muted-foreground text-sm">Aucun cours prévu aujourd'hui</p>
                        </div>
                    )}
                </div>
            )}
            ...
            {/* Activités Récentes */}
            <div className="space-y-3">
                <h2 className="font-bold text-lg px-1">Activités récentes</h2>
                {loadingExtra ? (
