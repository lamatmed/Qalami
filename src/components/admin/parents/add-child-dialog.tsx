...
export function AddChildDialog({ open, onOpenChange, parentId, parentName, onSuccess }: AddChildDialogProps) {
    ...
    const handleAdd = async (studentId: string, studentName: string) => {
        ...
        toast.success(`${studentName} ajouté(e) au compte de ${parentName}`)
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="bg-[#1A2530] border-white/10 text-white max-w-md">
                ...
                <p className="text-xs text-gray-400">Parent : <span className="text-white font-bold">{parentName}</span></p>
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                    {results.length === 0 && !searching && query && (
                        <p className="text-xs text-gray-500 text-center py-4">Aucun élève trouvé</p>
                    )}
                    {results.map(student => (
                        <div key={student.id} className="flex items-center justify-between p-3 bg-[#0F1720] rounded-xl border border-white/5">
                            <div>
                                ...
                                {student.national_id && <p className="text-xs text-gray-500 font-mono">NNI: {student.national_id}</p>}
                            </div>
                            <Button type="button" size="sm"
                                ...
                                {adding === student.id ? <Loader2 className="w-3 h-3 animate-spin" /> : 'Ajouter'}
                            </Button>
                        </div>
                    ))}
                </div>
                ...
                <Button variant="outline" className="w-full border-white/10 text-gray-400 hover:text-white"
                    onClick={() => onOpenChange(false)}>Fermer</Button>
            </DialogContent>
        </Dialog>
    )
}
