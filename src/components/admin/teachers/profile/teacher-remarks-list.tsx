...
export function TeacherRemarksList({ teacherId }: { teacherId: string }) {
    ...
    if (remarks.length === 0) return (
        <div className="text-center py-20 bg-[#1A2530] rounded-3xl border border-white/5">
            ...
            <p className="text-gray-500 font-medium">Aucune remarque enregistrée</p>
            <p className="text-xs text-gray-600 mt-1">Les commentaires laissés par cet enseignant apparaîtront ici.</p>
        </div>
    )
    ...
    return (
        <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-4 gap-3">
                {[
                    { key: 'positive',   label: 'Positifs',     count: stats.positive,   color: 'text-emerald-400', border: 'border-emerald-500/20' },
                    { key: 'negative',   label: 'Négatifs',     count: stats.negative,   color: 'text-red-400',     border: 'border-red-500/20' },
                    { key: 'behavioral', label: 'Comportement', count: stats.behavioral, color: 'text-orange-400',  border: 'border-orange-500/20' },
                    { key: 'academic',   label: 'Académiques',  count: stats.academic,   color: 'text-blue-400',    border: 'border-blue-500/20' },
                ].map(s => (
            ...
            <div className="bg-[#1A2530] rounded-2xl border border-white/5 px-5 py-3 flex justify-between items-center">
                <p className="text-sm text-gray-400">Total des remarques</p>
                <p className="text-lg font-black text-white">
                    {stats.total} <span className="text-xs text-gray-500 font-normal">remarque{stats.total !== 1 ? 's' : ''}</span>
                </p>
            </div>
            ...
            <div className="flex gap-2 flex-wrap">
                {([
                    ...
                    { key: 'positive',   label: 'Positifs',     count: stats.positive },
                    { key: 'negative',   label: 'Négatifs',     count: stats.negative },
                    { key: 'behavioral', label: 'Comportement', count: stats.behavioral },
                    { key: 'academic',   label: 'Académiques',  count: stats.academic },
                ...
                <div className="bg-[#1A2530] rounded-3xl border border-white/5 overflow-hidden">
                    <div className="divide-y divide-white/5">
                        {filtered.map(r => {
                            ...
                            const typeLabel  = TYPE_LABELS[typeKey] ?? r.type ?? 'Remarque'
                            return (
