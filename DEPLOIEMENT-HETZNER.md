...
## 16. Déploiement rapide — État actuel (qalami.avm.ai)

> **Contexte** : le serveur Hetzner est déjà en place et Supabase tourne via `supabase start` (Docker, CLI Supabase).
> Cette section donne les commandes exactes pour mettre Qalami en ligne sur `qalami.avm.ai`.

### 16.1 Vérifier l'état de Supabase sur le serveur

```bash
ssh -p 2222 deploy@<SERVER_IP>

# Récupérer l'URL, l'anon key et la service role key
supabase status
# Exemple de sortie :
...
# Noter le nom du réseau Docker créé par supabase start
docker network ls | grep supabase
# → supabase_network_qalami  (ou similaire)
```

Garder ces valeurs — elles seront utilisées dans `.env.production`.

### 16.2 DNS — ajouter les records dans votre zone `avm.ai`

Dans Cloudflare (ou le gestionnaire DNS de `avm.ai`) :

| Type | Nom | Contenu | Proxy |
| --- | --- | --- | --- |
| A | `qalami` | `<SERVER_IP>` | Proxied (nuage orange) |
| A | `api.qalami` | `<SERVER_IP>` | Proxied (nuage orange) |

Résultat : `qalami.avm.ai` → Next.js, `api.qalami.avm.ai` → Supabase API.

> Si `avm.ai` est géré ailleurs (OVH, Gandi…), créer les mêmes A records dans l'interface DNS de ce registrar.

### 16.3 Préparer le code en local

Ces étapes sont déjà faites dans le repo :

- [x] `output: 'standalone'` ajouté dans `next.config.ts`
- [x] `Dockerfile` créé à la racine

Il reste à créer les fichiers Docker Compose et `.env.production` **sur le serveur**.

### 16.4 Transférer le code sur le serveur

```bash
# Depuis votre machine locale (répertoire du projet)
cd d:/Nextjs/ADST/product-qalami-frontend-dev
...
### 16.5 Créer le fichier `.env.production` sur le serveur

```bash
ssh -p 2222 deploy@<SERVER_IP>

# Remplacer les valeurs par celles de `supabase status`
cat > /opt/qalami/app/.env.production << 'EOF'
NEXT_PUBLIC_SUPABASE_URL=https://api.qalami.avm.ai
NEXT_PUBLIC_SUPABASE_ANON_KEY=<ANON_KEY_de_supabase_status>
SUPABASE_SERVICE_ROLE_KEY=<SERVICE_ROLE_KEY_de_supabase_status>
NODE_ENV=production
NEXT_TELEMETRY_DISABLED=1
EOF
```

> **Pourquoi `https://api.qalami.avm.ai` et pas `http://localhost:54321` ?**
> L'URL `NEXT_PUBLIC_*` est embarquée dans le bundle JS envoyé au navigateur. Le navigateur ne peut pas atteindre `localhost:54321` du serveur — il faut lui donner l'URL publique.

### 16.6 Créer le Docker Compose de l'application

```bash
# Récupérer le nom exact du réseau Supabase
SUPABASE_NETWORK=$(docker network ls --format '{{.Name}}' | grep supabase | head -1)
echo "Réseau Supabase : $SUPABASE_NETWORK"

cat > /opt/qalami/app/docker-compose.yml << EOF
services:
  qalami:
    build:
      context: .
...
### 16.7 Configurer Caddy

```bash
mkdir -p /opt/qalami/caddy

cat > /opt/qalami/caddy/Caddyfile << 'EOF'
{
    email admin@avm.ai
}
...
cat > /opt/qalami/caddy/docker-compose.yml << 'EOF'
services:
  caddy:
    image: caddy:2-alpine
    container_name: qalami-caddy
...
> **Pourquoi `network_mode: host` pour Caddy ?** Caddy doit atteindre à la fois `qalami-app` (réseau Docker) et Supabase sur `localhost:54321` (port hôte). Avec `host`, il voit les deux sans configuration complexe. — **Alternative sans `host`** : utiliser un réseau partagé et router `api.qalami.avm.ai` vers le nom du conteneur Kong (`supabase_kong_qalami:8000`).

### 16.8 Lancer l'application

```bash
# 1. Builder et démarrer Qalami
cd /opt/qalami/app
docker compose build
docker compose up -d
...
# 3. Vérifier les logs Caddy (certificats TLS)
docker logs -f qalami-caddy
# → Attendre "certificate obtained successfully" pour les deux domaines

# 4. Vérifier l'application
curl -sf https://qalami.avm.ai > /dev/null && echo "OK" || echo "DOWN"
curl -sf https://api.qalami.avm.ai/rest/v1/ \
  -H "apikey: <ANON_KEY>" && echo "Supabase OK"
```

### 16.9 Appliquer les migrations de base de données

```bash
cd /opt/qalami  # ou là où est installé supabase

# Appliquer le schéma initial (si c'est un nouveau serveur)
supabase db push
...
Si Supabase est géré manuellement (sans CLI sur le serveur) :

```bash
# Copier et appliquer le fichier de migration initial
scp -P 2222 supabase/migrations/20260101000000_initial_schema.sql deploy@<SERVER_IP>:/tmp/

ssh -p 2222 deploy@<SERVER_IP> \
  'docker exec -i supabase-db-qalami psql -U postgres -d postgres < /tmp/20260101000000_initial_schema.sql'
```

### 16.10 Vérification finale
...
# Test complet
echo "--- App ---"
curl -sf https://qalami.avm.ai > /dev/null && echo "✓ qalami.avm.ai" || echo "✗ DOWN"
echo "--- API ---"
curl -sf https://api.qalami.avm.ai/rest/v1/ -H "apikey: <ANON_KEY>" > /dev/null && echo "✓ api.qalami.avm.ai" || echo "✗ DOWN"
echo "--- Auth ---"
curl -sf https://api.qalami.avm.ai/auth/v1/health > /dev/null && echo "✓ Auth" || echo "✗ DOWN"
```

### 16.11 Redéployer après une mise à jour du code
...
# Sur le serveur — rebuild et redémarrage
ssh -p 2222 deploy@<SERVER_IP> \
  'cd /opt/qalami/app && docker compose build && docker compose up -d'
```
