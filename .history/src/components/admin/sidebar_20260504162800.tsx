...
export function AdminSidebar() {
    ...
    useEffect(() => {
        if (groups.length > 0 && openGroups.length === 0) {
            setOpenGroups(groups.map(g => g.label))
        }
    ...
    return (
        <aside className={cn(
            "hidden lg:flex space-y-5 max-w-[1400px] mx-auto sticky top-0 bg-background z-30",
            ...
            <nav className="flex-1 overflow-y-auto py-3 px-3 flex flex-col scrollbar-hide">
                <div className="space-y-4">
                    {groups.filter(g => g.label !== t('admin.sidebar.system')).map((group) => {
