...
export function AdminSidebar() {
    ...
    // Initialize group order and open groups
    useEffect(() => {
        if (filteredGroups.length > 0 && groupOrder.length === 0) {
            setGroupOrder(filteredGroups.map(g => g.id))
            setOpenGroups(filteredGroups.map(g => g.id))
        }
    }, [filteredGroups, groupOrder.length])
    ...
    const moveGroupUp = (groupId: string) => {
        ...
        if (currentIndex > 0) {
            ...
            localStorage.setItem('sidebarGroupOrder', JSON.stringify(newOrder))
        }
    }
    ...
    useEffect(() => {
        const savedOrder = localStorage.getItem('sidebarGroupOrder')
        if (savedOrder) {
            try {
                const parsedOrder = JSON.parse(savedOrder)
                if (Array.isArray(parsedOrder) && parsedOrder.length > 0) {
                    setGroupOrder(parsedOrder)
                }
            } catch (e) {
        ...
        <aside className={cn(
            ...
            <nav className="flex-1 overflow-y-auto py-3 px-3 flex flex-col scrollbar-hide">
                <div className="space-y-4">
                    {sortedGroups.filter(g => g.id !== 'system').map((group) => {
                            ...
                            <div key={group.id} className="space-y-1 group">
                                <div className="flex items-center gap-1">
                                    <button
                                        ...
                                        className="flex-1 flex items-center justify-between px-2 mb-1 text-[11px] font-semibold text-muted-foreground/40 hover:text-muted-foreground transition-colors uppercase tracking-widest group/label"
                                    >
                                    ...
                                    {/* Boutons de déplacement - visible au survol */}
                                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button
                                            onClick={() => moveGroupUp(group.id)}
                                            disabled={groupOrder.indexOf(group.id) === 0}
                                            className={cn(
                                                "p-1 rounded hover:bg-muted transition-colors",
                                                groupOrder.indexOf(group.id) === 0 && "opacity-30 cursor-not-allowed"
                                            )}
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                "p-1 rounded hover:bg-muted transition-colors",
                                                groupOrder.indexOf(group.id) === groupOrder.length - 1 && "opacity-30 cursor-not-allowed"
                                            )}
                        ...
                        return (
                            <div key={group.id} className="space-y-1 group">
                                <div className="flex items-center gap-1">
                                    <button
                                        onClick={() => toggleGroup(group.id)}
                                        className="flex-1 flex items-center justify-between px-2 mb-1 text-[11px] font-semibold text-muted-foreground/40 hover:text-muted-foreground transition-colors uppercase tracking-widest"
                                    >
                                        <span>{group.label}</span>
                                        <ChevronDown className={cn(
                                            "h-3 w-3 transition-transform duration-200",
                                    ...
                                    {/* Boutons de déplacement pour system group */}
                                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button
                                            onClick={() => moveGroupUp(group.id)}
                                            disabled={groupOrder.indexOf(group.id) === 0}
